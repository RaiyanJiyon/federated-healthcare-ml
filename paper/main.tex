...
\begin{abstract}
 ...
 Baseline centralized approach results in an AUROC (Area Under the Receiver Operating Characteristic Curve) of 0.8920, which means that the method produces a recall of 85.2\%. Federated XGBoost approach using soft voting leads to an AUROC of 0.9155 (-1\% difference), while federated logistic regression using Platt scaling produces an AUROC of 0.8784. Robustness to the presence of Byzantine faults with Median and Krum algorithms allows obtaining AUROC $>$ 0.8594 with 3-of-7 advisory attack against clients compared to AUROC obtained with FedAvg that is equal to 0.5011 (-43\% difference). DP-SGD algorithm results in an AUROC value of 0.8477 after applying scaling operations. Federated learning algorithms can generalize as external validation using eICU-CRD (eICU Collaborative Research Database) dataset produces an AUROC of 0.8.
